...
export default function Footer() {
  return (
    <footer className="flex h-270 bg-spec-yellow-600 p-10">
      ...
      <div className="w-2/3 grid content-center pl-5">
        <div className="grid grid-rows-3 grid-cols-3 leading-10">
          ...
          <div><Link href="/">JoER</Link></div>
          <div><Link href="/">Code of Conduct</Link></div>
          <div><Link href="/">Get Involved</Link></div>
          <div><Link href="/">Documentation</Link></div>
          <div><Link href="/">Privacy Policy</Link></div>
        </div>
        <div className="font-bold mt-10">© 2022 Sustailable Progress & Equality Collective </div>
      </div>
    </footer>
  )
};
